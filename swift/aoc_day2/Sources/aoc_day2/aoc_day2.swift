...
struct Main: ParsableCommand {
  @Option(help: "red count to check")
  var red: UInt = 12
  ...
  private func sumOfPossibleGames(input: String) -> UInt {
    let gameIds = input
      ...
      .filter { $0.isPossible(redCount: red, greenCount: green, blueCount: blue) }
      .compactMap { $0.gameId }

    print("Game Ids: \(gameIds)")
...
func makeGame(line: String) -> Game {
  let game = Game()
  GameExpression(phrase: line).interpret(context: game)
  return game
}
